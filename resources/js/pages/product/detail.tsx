import { FlavorSelector } from "@/components/ui/flavor-selector"
...
export default function ProductDetailPage({ id }: { id: string }) {
    ...
    const handleAddToCart = () => {
        console.log("Adding to cart:", { product: product.name, flavor: selectedFlavor, quantity })
        alert(`Added ${quantity} ${product.name} (${selectedFlavor}) to cart! ✨`)
    }
        ...
        <CustomerLayout title={`${product.name} ✨`}>
            ...
            <div className="flex items-center justify-between mb-4">
                <button
                    ...
                    className="w-10 h-10 bg-linear-to-br from-cream to-rose-light rounded-full flex items-center justify-center hover:scale-110 hover:shadow-lg active:scale-95 transition-all duration-300 border border-brand-pink/20 cursor-pointer"
                >
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-4">
                            <button
                                ...
                                disabled={quantity <= 1}
                                className="w-12 h-12 bg-linear-to-br from-cream to-rose-light rounded-full flex items-center justify-center hover:scale-110 hover:shadow-lg active:scale-95 transition-all duration-300 shadow-md disabled:opacity-40 disabled:hover:scale-100 border border-brand-pink/20 cursor-pointer"
                            >
                            ...
                            <button
                                ...
                                className="w-12 h-12 bg-linear-to-br from-brand-pink to-rose-medium rounded-full flex items-center justify-center hover:scale-110 hover:shadow-[0_10px_30px_rgba(255,105,180,0.4)] active:scale-95 transition-all duration-300 shadow-lg text-white border border-brand-pink/40 cursor-pointer"
                            >
            ...
            {/* Sticky CTA */}
            <div className="fixed bottom-0 left-0 right-0 bg-linear-to-t from-bg-primary via-bg-primary to-transparent pt-6 pb-8 px-6 lg:static lg:bg-transparent lg:p-0 lg:pt-8">
                <div className="max-w-7xl mx-auto">
                    <JellyButton onClick={handleAddToCart} size="lg" className="w-full text-lg cursor-pointer">
                        Add to Basket 💖 ✨
                    </JellyButton>
                </div>
            </div>
