...
import { Heart, ChevronRight, Bell, Globe, CreditCard, LogOut, Package } from "lucide-react"
...
export default function ProfilePage() {
  ...
  return (
    <div className="min-h-screen bg-linear-to-b from-bg-primary via-rose-light to-bg-primary pb-32 px-4">
      {/* Header */}
      ...
      {/* Profile Card */}
      <SoftCard className="mb-6 bg-linear-to-br from-cream to-rose-light/50 border border-brand-pink/20 shadow-[0_10px_30px_rgba(255,105,180,0.15)]">
        <div className="flex items-center gap-4 p-6">
          ...
          <button className="text-brand-pink hover:scale-125 hover:rotate-90 transition-all duration-300">
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-4 px-2">
          ...
          <button className="text-brand-pink text-sm font-bold hover:scale-110 transition-transform px-3 py-1 rounded-full bg-brand-pink/10 hover:bg-brand-pink/20">View All ✨</button>
        </div>
        <div className="space-y-3">
          {orders.map((order) => (
            <SoftCard key={order.id} className="hover:scale-[1.02] hover:shadow-[0_8px_25px_rgba(255,105,180,0.2)] transition-all cursor-pointer bg-linear-to-r from-cream to-rose-light/30 border border-brand-pink/10">
              <div className="p-4">
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-3 gap-3">
          {favorites.map((item) => (
            <SoftCard
              key={item.id}
              className="hover:scale-105 transition-all cursor-pointer hover:shadow-[0_8px_25px_rgba(255,105,180,0.2)] bg-linear-to-br from-cream to-rose-light/40 border border-brand-pink/10"
              onClick={() => router.push(`/product/${item.id}`)}
            >
              <div className="relative aspect-square rounded-2xl overflow-hidden mb-2 border border-brand-pink/20">
                <Image src={item.image || "/placeholder.svg"} alt={item.name} fill className="object-cover" />
                <button
                  className="absolute top-2 right-2 w-8 h-8 bg-white rounded-full flex items-center justify-center shadow-lg hover:scale-125 hover:rotate-180 transition-all duration-300 border border-brand-pink/20"
                  onClick={(e) => {
                    e.stopPropagation()
      ...
      <div className="mb-6">
        ...
        <SoftCard className="bg-linear-to-br from-cream to-rose-light/30 border border-brand-pink/10">
            ...
            {settings.map((setting, index) => {
              ...
              return (
                <button
                  ...
                  className="w-full flex items-center justify-between p-4 hover:bg-brand-pink/10 hover:shadow-inner transition-all duration-300"
                >
